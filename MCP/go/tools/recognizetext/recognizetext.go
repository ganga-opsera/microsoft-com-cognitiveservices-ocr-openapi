...
func RecognizetextHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/recognizeText%s", cfg.BaseURL, queryString)
...
func CreateRecognizetextTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_recognizeText",
		mcp.WithDescription("Recognize Text operation. When you use the Recognize Text interface, the response contains a field called 'Operation-Location'. The 'Operation-Location' field contains the URL that you must use for your Get Recognize Text Operation Result operation."),
		mcp.WithString("mode", mcp.Required(), mcp.Description("Type of text to recognize.")),
